...
def balanced(m):
    ...
    """
    """if is_mobile(m):
        return balanced(left(m)) == balanced(right(m))
    elif is_arm(m):
        if is_planet(end(m)):
            return length(m) * total_weight(end(m))
        else:
    ...
    """if is_arm(m):
        if is_planet(end(m)):
            ...
            new_mobile = mobile(left(m), right(m))
            return balanced(new_mobile)
    elif is_mobile(m):
        return balanced(left(m)) == balanced(right(m))"""

    if is_planet(m):
        return True
    l = length(left(m)) * total_weight(end(left(m)))
    r = length(right(m)) * total_weight(end(right(m)))
    if l == r:
...
def preorder(t):
    ...
    [2, 4, 6]
    """
    #take the recursive leap of fate
    if is_leaf(t):
        return [label(t)]
    lst = [label(t)]
    for b in branches(t):
...
def has_path(t, word):
    ...
    if label(t) == word[0]:
        ...
        return my_word == word
    return any([has_path(b, word) for b in branches(t)) """
