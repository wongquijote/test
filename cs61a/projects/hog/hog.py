...
def free_bacon(score):
    ...
    >>> free_bacon(0)  # 1st digit of pi plus 3 (3 + 3).
    6
    >>> free_bacon(1)  # 2nd digit of pi plus 3 (1 + 3).
    4
    """
...
def swine_align(player_score, opponent_score):
    ...
    """
    # BEGIN PROBLEM 4a
    i, lagger, leader = 10, min(player_score, opponent_score), max(player_score, opponent_score)
    while(i <= lagger):
        if(lagger % i == 0 and leader % i == 0):
            return True
        i += 1
    return False
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while max(score0, score1) < goal: 
        if who == 0:
            ...
            get_extra_turn = extra_turn(score0, score1)
        else:
            score1 += take_turn(strategy1(score1, score0), score0, dice)
            get_extra_turn = extra_turn(score1, score0)
        if not get_extra_turn:
            who = other(who)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def announce(score0, score1):
        lastScore, runningHigh = last_score, running_high
        if who == 0:
            current_score = score0 - lastScore
            lastScore = score0
        elif who == 1:
            current_score = score1 - lastScore
            lastScore = score1
        if current_score > runningHigh: 
            print(current_score, 'point(s)! The most yet for Player', who)
            runningHigh = current_score
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    """
    # BEGIN PROBLEM 9
    count, highest_avg, max_dice = 1, 0, 0
    avg = make_averaged(roll_dice, trials_count)
    while count < 11:
        roll_avg = avg(count, dice)
        if(roll_avg > highest_avg):
            highest_avg = roll_avg
            max_dice = count
        count += 1
    return max_dice
    # END PROBLEM 9
